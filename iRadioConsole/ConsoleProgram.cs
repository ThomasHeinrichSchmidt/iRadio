...
namespace iRadio
{
    ...
    // TODO: add preselect keys's captions to user.config, update when doing SetToolTip()
    ...
    // ========================
    // DONE: README.md: add NoxonRemoteLetters.jpg (updated with current keys)
    ...
    // DONE: make form sizable (correctly dock elements), 
    // DONE: replace  Program.form.Invoke() with Program.form?.Invoke()
    // DONE: add settings to user.config (timeout for timeoutTimer (5000), last IP address used)
    // DONE: allow non-letter/digit keys (also on Remote) during NOXON search (textBoxSearch.Visible, Noxon.textEntry)
    // DONE: iRadio: add tool tips with id="artist" to preselect keys [1], [2],... see toolTip1.Show(Noxon.currentArtist, b); - probably needs some delay to be reliable
